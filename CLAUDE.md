...
**Preferred: local fastlane, one command per platform** (`mobile/fastlane/`, added 2026-07-08).
Run from `mobile/`: `fastlane android release` (builds AAB → uploads to Google Play `production`)
and `fastlane ios release` (builds IPA → uploads to **TestFlight**; you promote to the App Store
manually). Signing stays local (Android `key.properties`, iOS automatic signing); release notes come
from `fastlane/metadata/…/changelogs/default.txt` (Android) and are entered at App Store promotion
time (iOS). Credentials are gitignored and NOT in the repo — a Google Play service-account JSON
