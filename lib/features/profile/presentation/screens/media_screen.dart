...
class _MediaSectionState extends ConsumerState<_MediaSection> {
  ...
  Future<void> _pickAndUpload() async {
    ...
    final XFile? picked = widget.isVideo
    ...
    try {
      ...
      await repo.uploadFile(userId: userId, filePath: picked.path, type: widget.fileType);
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text(widget.title, style: DSTextStyle.headingSm.copyWith(fontSize: 15, fontWeight: FontWeight.w700, color: _c.text.primary)),
        const SizedBox(height: DSSpacing.s1),
        Text(widget.subtitle, style: DSTextStyle.bodySm.copyWith(color: _c.text.muted)),
        const SizedBox(height: DSSpacing.s3),
        Wrap(
          spacing: DSSpacing.s3,
          runSpacing: DSSpacing.s3,
          children: [
            ...widget.files.map((f) => _MediaTile(
              file: f,
...
class _AddTile extends StatelessWidget {
  const _AddTile({required this.isVideo, required this.isLoading, required this.onTap});
            ...
            : Column(
                ...
                children: [
                  Icon(isVideo ? LucideIcons.video : LucideIcons.imagePlus,
                      size: 28, color: onTap != null ? _c.text.secondary : _c.border.subtle),
                  const SizedBox(height: DSSpacing.s1),
                  Text(
                    isVideo ? 'Tilføj video' : 'Tilføj billede',
                    style: DSTextStyle.bodySm.copyWith(
                      fontSize: 10,
                      color: onTap != null ? _c.text.secondary : _c.border.subtle,
                    ),
                  ),
                ],
              ),
